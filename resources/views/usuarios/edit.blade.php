        ...
        <div class="card card-form">
            ...
            <div class="card-body p-4">
                ...
                <form method="POST" action="{{ route('usuarios.update', $user) }}" autocomplete="off">
                    ...
                    <div class="section-title">Configuración Médica</div>
                    ...
                    <div class="grid-2 mb-3">
                        ...
                        <div id="especialidadDiv">
                            ...
                            <select name="especialidad_id" class="form-select">
                                ...
                                @foreach ($especialidades as $esp)
                                    <option value="{{ $esp->id }}"
                                        {{ $user->especialidad_id == $esp->id ? 'selected' : '' }}>
                                        {{ $esp->nombre }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="consultorio_id" class="form-select">
                                ...
                                @foreach ($consultorios as $consultorio)
                                    <option value="{{ $consultorio->id }}"
                                        {{ $user->consultorio_id == $consultorio->id ? 'selected' : '' }}>
                                        {{ $consultorio->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if ($user->roles->contains('name', 'secretaria') || $user->roles->contains('name', 'recepcionista'))
                            <div>
                                ...
                                <select name="doctor_id" class="form-select">
                                    ...
                                    @foreach ($doctores as $doctor)
                                        <option value="{{ $doctor->id }}"
                                            {{ $user->doctor_id == $doctor->id ? 'selected' : '' }}>
                                            {{ $doctor->name }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleFields() {
                ...
                consultorioDiv.style.display = 'block';
            }
